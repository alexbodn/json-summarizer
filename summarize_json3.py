...
def analyze_types(lst):
    ...
    for e in lst:
        if type(e) is dict:
            ...
            ret['string'] += 1
        elif type(e) in (int, long):
            ret['int'] += 1
        elif type(e) in (float, ):
            ret['float'] += 1
        elif type(e) is bool:
            ret['bool'] += 1
        else:
...
# returns 'dict', 'list', 'string', 'int', 'float', 'bool', 'null'
...
def summarize_list(lst, indent, is_toplevel=False):
    ...
    if len(lst) == 0:
        ...
        if ht == 'dict':
            ...
            print (prefix + ht,)
            summarize_list_of_ints(lst, indent+1)
        elif ht == 'float':
            print (prefix + ht,)
            summarize_list_of_floats(lst, indent+1)
        elif ht == 'string':
            print (prefix + ht)
...
def summarize_list_of_dicts(lst, indent):
    ...
    for f, c in field_counts.items():
        k = spaces(indent+1) + '> ' + repr(str(f))
        if c < total:
            sublist = [e[f] for e in lst if f in e]
            k += ' (%d of %d records) ' % (c, total)
        else:
...
def summarize_list_of_ints (lst, indent):
    lst_min = min(lst)
    lst_max = max(lst)
    ...
    # singleton
    if len(set(lst)) == 1:
        print (spaces(indent+1) + str(lst[0]))
        return

    print (spaces(indent+1) + '[min: %d, mean: %d, max: %d]' % (lst_min, lst_mean, lst_max),)

    # test sortedness or reverse sortedness
    if sorted(lst) == lst:
        print (spaces(indent+1) + 'sorted')
    elif reversed(lst) == lst:
        print (spaces(indent+1) + 'reverse sorted')
    else:
        print


def summarize_list_of_floats(lst, indent):
    lst_min = min(lst)
    lst_max = max(lst)
    lst_mean = float(sum(lst)) / len(lst)
    ...
    print (spaces(indent+1) + '[min: %.4g, mean: %.4g, max: %.4g]' % (lst_min, lst_mean, lst_max),)
...
def summarize_list_of_strings(lst, indent):
    ...
    if len(set(hist.values())) == 1 and 1 in hist.values():
        print (spaces(indent+1), len(lst), 'distinct values')
        return
